...
const thankDiv= document.querySelector('.thanks-div')
...
function updateDisplays() {
  ...
  fiveDisplay.innerHTML = `${five} <sup class="h3-span">left</sup>`;
  sevenDisplay.innerHTML = `${seven} <sup class="h3-span">left</sup>`;
}
...
selectDiv.querySelector('.five').innerHTML = `${remainingItems} <sup class="h3-span">left</sup>`;
    twentyInput.value = ''; // Clear the input field
  } else {
...
selectDiv.querySelector('.seven').innerHTML = `${remainingItems} <sup class="h3-span">left</sup>`;
    seventyInput.value = ''; // Clear the input field
  } else {
    alert('Please enter money more than minimum amount');
  }
});
